...
export async function generateMetadata({ searchParams }: PageProps<'/shop'>): Promise<Metadata> {
  ...
  const key = isFilterKey(typeof filter === 'string' ? filter : undefined) ? (filter as FilterKey) : 'all';
  return { title: `Shop ${TITLES[key]}`, description: INTROS[key] };
}

export default async function ShopPage({ searchParams }: PageProps<'/shop'>) {
  const { filter } = await searchParams;
  const active: FilterKey = isFilterKey(typeof filter === 'string' ? filter : undefined) ? (filter as FilterKey) : 'all';
      ...
      <main id="main" className={`${styles.main} container`}>
        ...
        {active === 'sets' ? (
          <div className={styles.setGrid}>
            {sets.map((set, index) => (
              <SetCard key={set.top.handle} set={set} delay={(index % 3) * 70} />
            ))}
          </div>
        ) : (
          <div className={styles.grid}>
            {products.map((product, index) => (
              <ProductCard key={product.handle} product={product} priority={index < 4} delay={(index % 4) * 60} />
            ))}
          </div>
        )}
      </main>
