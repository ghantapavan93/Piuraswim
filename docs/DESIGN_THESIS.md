...
Next.js 16, server components by default, small client islands. Route groups give editorial pages a floating header, shopping pages a solid one. Tokens in one global stylesheet; each component owns a stylesheet with readable block__element class names, so the inspector reads like the source. Images render as plain `<img>` with next/image's srcset and no inline styles. Reveals are one attribute on the semantic element, observed once from the layout: no wrappers, content visible without JavaScript. Catalog and image manifest are generated from the storefront snapshot. Product JSON-LD with an offer per size, generated sitemap and robots, native disclosures and progress, radio-group sizes, focus management in drawers, a skip link.
